...
import {userLogin,getClassify,getGoods,getShopList,getCarousel,getAddr,getOrder,mine,offline,factory,ruzhi,shangjin,news,newsxq,Storedelivery,tixianrecord,tansrecord} from '../services/shop';
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        ...
        //   })
        // }
        else if(location.pathname === '/Presentrecord'){
          //入职记录
          dispatch({
            type:'inrecord',   
            payload:{is_hot: true, page: 1,size:12}
          })
        }else if(location.pathname === '/Reward'){
          //雅虎赏金
          dispatch({
            type:'shangjin',   
            payload:{page: 1,size:12,type:1}
          })
        }else if(location.pathname === '/Foundmanage'){
          //奖金明细
          dispatch({
            type:'shangjin',   
            payload:{page: 1,size:12,type:0}
          })
        }
        ...
        }else if(location.pathname === '/Xieyi'){
          dispatch({
            type: 'pack',
            payload:""
          })
  ...
  effects: {
    ...
    *shangjin({ payload}, {call, put ,select}) {
      ...
      *off({ payload}, {call, put ,select}) {
        let List = yield select(state => state.shop.offinfo);
        if(!payload||payload.page==1){
          List=[];
          yield put({
            type: 'getUser',
            payload: {offinfo:List}
          })
        }
        // 发送请求
        ...
        if (data.code==1) {
          let {current_page,last_page,per_page,total}=data.data;
          let offinfo=List.concat(data.data.data);
          yield put({
            type: 'updateList',
            payload: {
              offinfo,
    ...
    *xieyi({ payload }, {call, put}) {
    ...
    //已领取礼包
    // *ypack({ payload }, {call, put}) {
    //   const data = yield call(shop.yypack,payload);
    //   console.log(data,'@@@@111')
    //   if (data.code==1) {
    //     yield put({
    //       type: 'getUser',
    //       payload:{ypackInfo: data.data} 
    //       // console.log(userInfo,'********')
    //     })
    //   }else{
    //     Toast.fail(data.msg, 2);
    ...
    // },
      ...
      //   const data = yield call(shop.Storedelivery,payload);
      //   console.log(data,'@@@@111')
      //   if (data.code==1) {
      //     yield put({
      //       type: 'getUser',
      //       payload:{storeInfo: data.data.data} 
      //       // console.log(userInfo,'********')
      //     })
      //   }else{
      //     Toast.fail(data.msg, 2);
      //   }
      // },
      // *newsxq({ payload}, {call, put ,select}) {
      //   let List = yield select(state => state.shop.newxqlist);
      //   if(!payload||payload.page==1){
      //     List=[];
      //     yield put({
      //       type: 'getUser',
      //       payload: {newxqlist:List}
      //     })
      //   }
      //   // 发送请求
      //   const data = yield call(newsxq,payload);
      //   if (data.code==1) {
      //     let {current_page,last_page,per_page,total}=data.data;
      //     let newxqlist=List.concat(data.data);
      //     yield put({
      //       type: 'updateList',
      //       payload: {
      //         newxqlist,
 ...
 *factory({ payload}, {call, put ,select}) {
  ...
  *team({ payload }, {call, put}) {
   ...
   //速聘经理
  //  *supin({ payload }, {call, put}) {
  //   const data = yield call(shop.supin,payload);
  //   console.log(data,'@@@@111')
  //   if (data.code==1) {
  //     yield put({
  //       type: 'getUser',
  //       payload:{supinInfo: data.data.data} 
  //       // console.log(userInfo,'********')
  //     })
  //   }else{
  //     Toast.fail(data.msg, 2);
  ...
  // },
   //雅虎经理
   *yahu({ payload }, {call, put}) {
    const data = yield call(shop.yahu,payload);
    console.log(data,'@@@@111')
    if (data.code==1) {
      yield put({
        type: 'getUser',
        payload:{yahuInfo: data.data.data} 
        // console.log(userInfo,'********')
      })
    }else{
      Toast.fail(data.msg, 2);
  ...
  //提现
  // *tixian({ payload }, {call, put}) {
  //   const data = yield call(shop.tixian,payload);
  //   // console.log(data,'@@@@111')
  //   if (data.code==1) {
  //     // console.log(data,"&&&&&&&&&&&&&&&&")
  //     yield put({
  //       type: 'getUser',
  //       payload:{tixainInfo: data.data} 
  //     })
  //   }else{
  //     Toast.fail(data.msg, 2);
  //   }
  // },
  //提现记录
 *tixianjl({ payload}, {call, put ,select}) {

  let List = yield select(state => state.shop.tixianList);
  
  if(!payload||payload.page==1){
    List=[];
    yield put({
      type: 'getUser',
      payload: {tixianList:List}
    })
  }
  // 发送请求
  const data = yield call(tixianrecord,payload);
  if (data.code==1) {

    let {current_page,last_page,per_page,total}=data.data;
   
    let tixianList=List.concat(data.data.data);
