...
import { SearchBar, Button, WhiteSpace, WingBlank,Toast } from 'antd-mobile';
...
export default class Teamlist2 extends Component {
  state = {
    value: '美食',
    DATA:'',
  ...
  async componentDidMount(){
    const dataInfo = await fetch.yahu({})
    ...
    Shop.yahu({name: value }).then((result) => {
...
render() {
  const yshulist=this.state.DATA;
  const navBarProps = {
    leftVisible: true,
    leftFunc(){
      history.go(-1)
  ...
  // const yshulist=shop.yahuInfo;
  console.log(yshulist,'ppppppppppppppppppppp')
